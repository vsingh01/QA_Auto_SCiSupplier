...
public class Browser {
    ...
    public static void initBrowserWebDriver() {
        ...
        String browserDriverDirectory = System.getProperty("user.dir") + File.separator + "Webdriver" + File.separator;
        switch (browser) {
            case "firefox":
                driver = getFirefoxDriver();
                break;
            case "chrome":
                System.setProperty("webdriver.chrome.driver", browserDriverDirectory + "chromedriver.exe");
                driver = getChromeDriver();
                break;
            case "explorer":
                System.setProperty("webdriver.ie.driver", browserDriverDirectory + "IEDriverServer.exe");
                driver = getIEDriver();
                break;
            default:
    ...
    public static void initGridDriver(String gridBrowser, String gridNodeIP, String gridNodePort) {
        setBrowserCapabilities(gridBrowser);
        try {
            driver = new RemoteWebDriver(new URL("http://" + gridNodeIP + ":" + gridNodePort + "/wd/hub"), capabilities);
    ...
    public static void initGrid(String gridBrowser, String gridNodeIP, String gridNodePort) {
    ...
    public static void setScreenSize(String aScreenSize) {
        ...
        if (Browser.SMALL.equalsIgnoreCase(aScreenSize)) {
            Browser.eDriver.manage().window().setSize(getBrowserDimension(Browser.SMALL));

        } else if (Browser.MEDIUM.equalsIgnoreCase(aScreenSize)) {
            Browser.eDriver.manage().window().setSize(getBrowserDimension(Browser.MEDIUM));
    ...
    public static Dimension getBrowserDimension(String aScreenSize) {
        ...
        return new Dimension(Integer.valueOf(parsedSize[0]), Integer.valueOf(parsedSize[1]));

    }

    public static boolean isGridModeOn(String gridBrowser, String gridNodeIP, String gridNodePort) {
        if ((gridBrowser == null) || (gridNodeIP == null) || (gridNodePort == null)) {
            return false;
        } else if (gridBrowser.isEmpty() || gridNodeIP.isEmpty() || gridNodePort.isEmpty()) {
            return false;
