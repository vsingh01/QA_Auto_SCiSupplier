...
public class EBrowser {
	...
	final static Logger logger = Logger.getLogger(EBrowser.class);
	...
	public static void setScreenSize(String aScreenSize) {
		...
		if (EBrowser.SMALL.equalsIgnoreCase(aScreenSize)) {
			EBrowser.eDriver.manage().window()
					.setSize(getBrowserDimension(EBrowser.SMALL));

		} else if (EBrowser.MEDIUM.equalsIgnoreCase(aScreenSize)) {
			EBrowser.eDriver.manage().window()
					.setSize(getBrowserDimension(EBrowser.MEDIUM));

		} else if (EBrowser.LARGE.equalsIgnoreCase(aScreenSize)) {
	...
	public static Dimension getBrowserDimension(String aScreenSize) {
		...
		if (aScreenSize.equalsIgnoreCase(EBrowser.SMALL)) {
			size = PRBase.setupProp.getProperty("browser.size.small");
		}

		if (aScreenSize.equalsIgnoreCase(EBrowser.MEDIUM)) {
			size = PRBase.setupProp.getProperty("browser.size.medium");
		}

		if (aScreenSize.equalsIgnoreCase(EBrowser.LARGE)) {
